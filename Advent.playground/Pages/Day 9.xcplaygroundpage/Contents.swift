...
func length(from input: String, length: Int) -> Int {
    ...
    return sequence(state: Scanner(string: input)) { (scanner: inout Scanner) -> (SIMD2<Int>, Int)? in
        scanner.charactersToBeSkipped = .alphanumerics.inverted
        guard let direction = scanner.scanCharacter(),
              let count = scanner.scanInt() else {
            return nil
        }
        switch direction {
        case "U": return (.init(x: 0, y: 1), count)
        case "D": return (.init(x: 0, y: -1), count)
        case "L": return (.init(x: -1, y: 0), count)
        case "R": return (.init(x: 1, y: 0), count)
        default: return nil
        }
    }
    .reduce(into: State(rope: .init(repeating: .zero, count: length))) { state, next in
        let (Δ, count) = next
