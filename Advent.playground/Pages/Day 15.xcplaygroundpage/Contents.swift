...
func total(from string: String, target: Int) -> (Int, Int) {
    ...
    let total1 = (minX...maxX).lazy.map { Point(x: $0, y: target) }.reduce(0) { total, target in
        total + (inputs.first { (sensor, beacon, manhattan) in
            sensor.manhattan(target) <= manhattan && beacon != target
        }.map { _ in 1 } ?? 0)
    }
...
print(total(from: makeInput(), target: 2000000))
//(5176944, 13350458933732)
