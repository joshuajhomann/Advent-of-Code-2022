...
let total = sequence(state: Scanner(string: input)) { scanner in
    ...
    guard let a = scanner.scanInt(),
        let b = scanner.scanInt(),
        let c = scanner.scanInt(),
        let d = scanner.scanInt() else {
        return nil
    }
    let isContained =
...
let Σ = ranges.map { (a,b) in
    let isSubset = a.contains(b.upperBound) && a.contains(b.lowerBound) ||
                   b.contains(a.upperBound) && b.contains(a.lowerBound)
    return isSubset ? 1 : 0
}.reduce(0, +)
print(Σ)


let Σ2 = ranges.map { $0.overlaps($1) ? 1 : 0 }.reduce(0, +)
print(Σ2)
